...
/*Function sends encrypted file size to the server, gets acknowledgement
...
/*Function that receives the decrypted file*/
void receiveDecFile(int sock, int fileSize){
...
int main(int argc, char *argv[]){
   ...
   if ((n = recv(sockfd, &num, sizeof(num), 0)) < -1){
      error("Client cannot get aknowledgment");
   }
   //check that acknowledgement from server is 6; it will mean 
   ...
   //if files were opened, send them if key > ciphertext  
   else{
      //get file size
      ...
      if (keySize < textSize){
         fprintf(stderr, "The key file cannot be shorter than the ciphertext.\n");
   ...
   //receive decrypted file from the server
   receiveDecFile(sockfd, textSize);
   //close socket
   close(sockfd);
   return 0;
}
