...
/*Function that receives the encrypted text and the key files,
 *performs decryption and sends decrypted file back to the client*/
void handleDec(int sock){
   ...
   //receive size of the encrypted text file; if failed show error 
   ...
   char *msg = "I received the ciphertext file size"; 
   ...
   while (sizeReceived+1 < fileSize){
      ...
      if ((n = recv(sock, buff, sizeof(buff), 0)) == -1){
         error("Cannot receive ciphertext file");
         exit(1);
      }
      else if (n == 0){
         exit(1);
   ...
   char *str = "I received the ciphertext file"; 
   ...
   //perform decryption
   int i;
   char decFile[fileSize]; //holds decrypted file 
   char current; 
   ...
   for (i = 0; fileContent[i] != '\n'; i++){
	  ...
	  //calculate decrypted character using mod 27
      ...
      decFile[i] = current; //add calculated character to decrypted file
   }
   decFile[i] = '\0'; //add null char to the end
   //send decrypted file to the client
   if ((n = send(sock, decFile, sizeof(decFile), 0)) == -1){
...
int main(int argc, char *argv[]){
   ...
   while(1){
      ...
      if (pid == 0){
         ...
         int authNum = 6;//authentication number for this server
         ...
         handleDec(newsockfd);//do decryption
         close(newsockfd); //close socket
         close(newsockfd);
         exit(0);
      } 
